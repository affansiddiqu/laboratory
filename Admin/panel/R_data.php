...
$query = "SELECT * FROM `form_data` WHERE received_by = 'Rizwan Ansari' AND status = '1' 
  ...
  <div class="container">
    ...
    <div class="row justify-content-left">
            ...
            <div class="col-xl-10 col-lg-12 col-md-9">
                <h2>Received By Rizwan Ansari</h2>
            ...
            <thead class="bg-secondary p-2 text-dark bg-opacity-10" style="opacity: 75%;">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><a href="aupdatedata.php?id=<?php echo $row['id']; ?>" class="btn btn-success">Update</a></td>
...
<?php
// Pagination logic
$paginationQuery = "SELECT * FROM `form_data` WHERE received_by = 'Rizwan Ansari' AND status = '1' 
                    AND (name LIKE ? OR investigation LIKE ? OR total_amount LIKE ?)";
$stmtPagination = mysqli_prepare($connection, $paginationQuery);
mysqli_stmt_bind_param($stmtPagination, 'sss', $searchTerm, $searchTerm, $searchTerm);
mysqli_stmt_execute($stmtPagination);
$resultPagination = mysqli_stmt_get_result($stmtPagination);

if(mysqli_num_rows($resultPagination) > 0){
    $totalRecords = mysqli_num_rows($resultPagination);
    $totalpages = ceil($totalRecords / $limit);
    echo '<ul class="pagination">';
    if($getpgno > 1){
        echo '<li class="page-item"><a class="page-link" href="R_data.php?page='.($getpgno - 1).'">prev</a></li>';
    }
    for($i = 1; $i <= $totalpages; $i++){
        $active = $i == $getpgno ? "active" : "";
        echo '<li class="'.$active.' page-item"><a class="page-link" href="R_data.php?page='.$i.'">'.$i.'</a></li>';
    }
    if($getpgno < $totalpages){
        echo '<li class="page-item"><a class="page-link" href="R_data.php?page='.($getpgno + 1).'">next</a></li>';
    }
    echo '</ul>';
}
?>
