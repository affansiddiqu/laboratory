...
$query = "SELECT * FROM `form_data` WHERE status = '1' 
...
<?php
// Pagination logic
$fetchpage = "SELECT * FROM `form_data` WHERE status = '1'";
...
if(mysqli_num_rows($query) > 0){
    ...
    if($getpgno > 1){
        echo '<li class="page-item"><a class="page-link" href="all.php?page='.($getpgno - 1).'">prev</a></li>';
    }
    for($i = 1; $i <= $totalpages; $i++){
        $active = $i == $getpgno ? "active" : "";
        echo '<li class="'.$active.' page-item"><a class="page-link" href="all.php?page='.$i.'">'.$i.'</a></li>';
    }
    if($getpgno < $totalpages){
        echo '<li class="page-item"><a class="page-link" href="all.php?page='.($getpgno + 1).'">next</a></li>';
    }
    echo '</ul>';
}
?>
